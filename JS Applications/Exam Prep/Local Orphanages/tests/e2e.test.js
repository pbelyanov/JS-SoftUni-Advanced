...
describe('E2E tests', function () {
  ...
  before(async () => (browser = await chromium.launch(DEBUG ? {
    headless: false,
    slowMo
  } : {})));
  ...
  describe('CRUD [ 50 Points ]', () => {
    ...
    it('Check details information [ 5 Points ]', async () => {
      ...
      const {
        get: own
      } = await handle(endpoints.own(data._id, user._id));
      const {
        get: total
      } = await handle(endpoints.total(data._id));
      own(0);
      total(5);
    ...
    it('Edit does NOT work with empty fields [ 5 Points ]', async () => {
      ...
      const {
        get,
        put
      } = await handle(endpoints.delete(data._id));
      get(data);
      const {
        isHandled
      } = put();
  ...
  describe('User Profile Page [ 10 Points ]', () => {
    ...
    it('Check profile page for with 0 posts [ 2.5 Points ]', async () => {
      const {
        get
      } = await handle(endpoints.profile(mockData.users[0]._id));
      get([]);
  ...
  describe('BONUS : Donate functionality  [ 15 Points ]', () => {
    ...
    it('Donate button should be hidden(not visible) after a click on it [ 2.5 Points ]', async () => {
      ...
      const {
        post
      } = await handle(endpoints.donate, {
        post: mockData.donations[2]
      });
      const {
        onRequest
      } = post({
        postId: mockData.donations[2].postId
      });
      own(0);
      total(5);
...
async function setupContext(context) {
  ...
  await handleContext(endpoints.profile('0001'), {
    get: mockData.catalog.slice(0, 2)
  }, context);

  await handleContext(endpoints.total('1001'), {
    get: 6
  }, context);
  await handleContext(endpoints.total('1002'), {
    get: 4
  }, context);
  await handleContext(endpoints.total('1003'), {
    get: 7
  }, context);

  await handleContext(endpoints.own('1001', '0001'), {
    get: 1
  }, context);
  await handleContext(endpoints.own('1002', '0001'), {
    get: 0
  }, context);
  await handleContext(endpoints.own('1003', '0001'), {
    get: 0
  }, context);
